<div id="main-content">
			...
			<p><strong>Essa é sua caixa de entrada, aqui ficam guardadas todas as mensagens que foram enviadas à você.</strong></p>
		</div>
...
<a href="index.php?view=pm&act=compose" class="btn btn-info">CRIAR MENSAGEM</a>
		...
		<?php 
		// BUSCA MSGS
		$msg_query = mysql_query("SELECT * FROM msg WHERE para='{$login}' ORDER BY id DESC");
		while($msg_data = mysql_fetch_array($msg_query)){
		?>
		...
		$buscamsg = mysql_query("SELECT * FROM msg WHERE id = '$msg_id'");
		$dadosmsg = mysql_fetch_array($buscamsg);
		
		if ($dadosmsg['para'] != $login) { echo "Essa mensagem não foi enviada para você"; } 
		else
		{
		$update = mysql_query("UPDATE msg SET status='read' WHERE id = '$msg_id'");
		?>
		<br>
		<div class="alert alert-block alert-success fade in">
		...
		if ($_GET['act'] == "compose")
		{
			...
			$dt = time();
			$inserir = mysql_query("INSERT INTO msg (id,de,para,assunto,texto,data,status) VALUES (NULL, '$login', '$recebe', '$assunto', '$texto', '$dt', 'unread');") or die (mysql_error());
